<template>
    ...
    <form class='space-y-12' @submit.prevent='submit'>
        ...
        <div class='space-y-4'>
            <FormInput id='email' v-model.trim.lazy='form.email' :error='errors?.email'
            ...
            <div class='relative flex items-start'>
                <div class='flex items-center h-5'>
                    <input id='remember' v-model.lazy='form.remember'
                           class='form-checkbox focus:ring-primary-500 h-4 w-4 text-primary-600 border-gray-300 rounded'
                           name='remember'
                           type='checkbox'>
                </div>
                <div class='ml-3 text-sm'>
                    <label class='font-medium text-gray-700 dark:text-primary-300'
                           for='remember'>{{ $trans('remember_me') }}</label>
                </div>
            </div>
        </div>
        <div class='flex flex-col'>
            ...
            <Link :href="route('auth.forgot-password.view')"
...
<script> import Logo from '@/Auth/Logo.vue'
