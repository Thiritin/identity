<template>
    <Head :title="$t('security_passkeys')" />
    <div>
        ...
        <SettingsHeader>{{ $t('security_passkeys') }}</SettingsHeader>
        <SettingsSubHeader class="mb-4">
            {{ passkeys.length > 0 ? $t('passkey_keys_registered', { count: passkeys.length }) : $t('passkey_register_subtitle') }}
        </SettingsSubHeader>

        <!-- Registered passkeys list -->
        <div v-if="passkeys.length > 0" class="divide-y divide-gray-200 dark:divide-gray-700 mb-6">
            <div v-for="key in passkeys" :key="key.id"
                 class="flex items-center justify-between py-3">
                <div>
                    <p class="font-medium text-sm text-gray-900 dark:text-gray-100">{{ key.name }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        {{ key.last_used_at ? $t('passkey_last_used', { date: key.last_used_at }) : $t('passkey_never_used') }}
                    </p>
                </div>
        ...
        <div v-if="removeId" class="mb-6 rounded-lg border border-destructive/50 p-4">
            <form @submit.prevent="submitRemove" class="space-y-4">
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('passkey_remove_password') }}</p>
                <div class="flex flex-col gap-2">
                    <label for="passkey_remove_password">{{ $t('password') }}</label>
                    <Input id="passkey_remove_password" type="password"
        ...
        <div v-if="!showAddForm">
            <Button variant="outline" @click="startRegistrationFlow">{{ $t('passkey_add_new') }}</Button>
        </div>
        <div v-else>
            <form @submit.prevent="submitAdd" class="space-y-4">
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('passkey_tap_to_register') }}</p>
                <div class="flex flex-col gap-2">
                    <label for="passkey_name">{{ $t('name') }}</label>
                    <Input id="passkey_name" type="text" placeholder="MacBook, iPhone, etc."
                ...
                <div class="flex justify-end gap-3">
                    <Button type="submit" :disabled="addForm.processing || !registrationResponse" class="order-2">{{ $t('passkey_register_key') }}</Button>
...
const addForm = useForm('post', route('settings.two-factor.passkey.store'), {
    credential: '',
    name: '',
})

const removeForm = useForm('delete', route('settings.two-factor.passkey.destroy'), {
    keyId: '',
    password: '',
})
...
async function startRegistrationFlow() {
    ...
    try {
        const optionsRes = await fetch(route('settings.two-factor.passkey.options'))
