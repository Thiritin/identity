...
RUN apk add --no-cache --virtual .build-deps curl-dev libzip-dev icu-dev libtool libxml2-dev oniguruma-dev postgresql-dev autoconf gcc make g++ zlib-dev libjpeg-turbo-dev libpng-dev freetype-dev  \
    && apk add --no-cache curl supervisor libzip openssl libintl icu git unzip zip tar ca-certificates dcron mysql-client freetype libzip libjpeg-turbo libpng \
    && pecl install redis \
    && docker-php-ext-enable redis \
    && docker-php-ext-configure gd --with-freetype --with-jpeg \
    && docker-php-ext-install gd bcmath curl iconv mbstring openssl pdo pdo_mysql tokenizer xml zip intl opcache pcntl \
