...
class OpenIDConnectClient
{
    ...
    public $laravelRedirectUrl;
    public Request $laravelRequest;
     ...
     * @param null $issuer
     */
    public function __construct($provider_url = null, $client_id = null, $client_secret = null, $issuer = null, Request $request)
    {
    ...
    public function authenticate()
    {
        ...
        if ($this->laravelRequest->get('error') !== null) {
            $desc = $this->laravelRequest->get('error_description') !== null ? ' Description: ' . $this->laravelRequest->get('error_description') : '';
            throw new OpenIDConnectClientException('Error: ' . $this->laravelRequest->get('error') . $desc);
        }

        // If we have an authorization code then proceed to request a token
        if ($this->laravelRequest->get('code') !== null) {

            $code = $this->laravelRequest->get('code');
            ...
            if ($this->laravelRequest->get('state') !== $this->getState()) {
        ...
        if ($this->allowImplicitFlow && $this->laravelRequest->get('id_token') !== null) {
            // if we have no code but an id_token use that
            $id_token = $this->laravelRequest->get('id_token');

            $accessToken = null;
            if ($this->laravelRequest->get('access_token') !== null) {
                $accessToken = $this->laravelRequest->get('access_token');
            }
